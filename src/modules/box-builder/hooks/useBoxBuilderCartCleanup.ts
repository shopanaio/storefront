...
export function useBoxBuilderCartCleanup() {
  ...
  const cleanup = useCallback((checkoutId: string | null) => {
    ...
    if (checkoutId) {
      ...
      console.log(`[Order Completion] Cleaned up box-builder cart, store, and removed checkout ${checkoutId} from Relay store`);
    } else {
      console.log('[Order Completion] Cleaned up box-builder cart and store');
    }
  }, [clearBoxBuilder, environment]);
