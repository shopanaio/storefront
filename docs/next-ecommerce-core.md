...
**Ключевые принципы:**
1.  **RSC First (Server Components):** Страница-обвязка и `PageBuilder` выполняются на сервере, HTML приходит в браузер готовым, а JS подключается только там, где он действительно нужен.
...
4.  **Islands Architecture:** Интерактивность (слайдеры, меню) добавляется точечно через `'use client'` компоненты внутри секций.
...
7.  **Client Sections:** Все секции — это клиентские компоненты с директивой `'use client'`, подключаемые через динамические импорты и работающие как острова поверх серверного `PageBuilder`.
...
> **Важно:** каждый компонент секции объявлен как клиентский (`'use client'`). Это позволяет секциям брать на себя интерактивность, в то время как сама страница и `PageBuilder` остаются серверными.
...
- ядро (`@shopana/next-ecommerce-core/core`): `PageBuilder`, `PageDataContext`, типы для секций/шаблонов/страниц;
- контекст магазина (`@shopana/next-ecommerce-core/shop`): `ShopProvider`, `useShop`, `ShopConfig`, ...;
- SDK (`@shopana/next-ecommerce-core/sdk`): entity-типы, Page Data-типы и client-хуки (`useProduct`, `useCollection`, ...), причём server SDK остаётся внутренним.
...
- Всё, что делает пользователь:
  - создаёт свои секции/блоки в `src/sections/**` (используя типы и хуки из `@shopana/next-ecommerce-core`);
...
export interface SectionInstance<TSettings = any> {
  id: string;
  component:
    | ComponentType<SectionProps<TSettings>>
    | React.LazyExoticComponent<ComponentType<SectionProps<TSettings>>>; // Ссылка на компонент (в т.ч. dynamic)
  settings: TSettings; // Настройки должны соответствовать T
}
